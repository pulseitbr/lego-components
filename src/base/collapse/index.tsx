...
class Collapse extends React.Component<Props, any> {
	...
	onRest = ({ isFullyOpened, isFullyClosed, isOpened, containerHeight, contentHeight }: any) => {
		if (!this.container || !this.content) {
			return;
		}

		const hasOpened = isOpened && (this.container! as any).style.height === `${contentHeight}px`;
		const hasClosed = !isOpened && (this.container! as any).style.height === "0px";

		if (hasOpened || hasClosed) {
			(this.container! as any).style.overflow = isOpened ? "initial" : "hidden";
	...
	onWork = ({ isFullyOpened, isFullyClosed, isOpened, containerHeight, contentHeight }: any) => {
		if (!this.container || !this.content) {
			return;
		}

		const isOpenining = isOpened && (this.container! as any).style.height === `${contentHeight}px`;
		const isClosing = !isOpened && (this.container! as any).style.height === "0px";
		...
		(this.container! as any).style.height = isOpened ? `${contentHeight}px` : "0px";
	...
	render() {
		...
		return (
			<div
				...
				style={{ ...this.initialStyle, transition: `height ${this.props.time}ms` }}
			>
				<div ref={(content: any) => Object.assign(this, { content })} className={theme.content}>
					{children}
				</div>
			</div>
		);
